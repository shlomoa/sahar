#!/usr/bin/env node
// SAHAR Validation Orchestrator (TypeScript)
// Usage:
//   node dist/src/validate.js quick            -> run default hook sequence (A,B,I,C,D,E,J)
//   node dist/src/validate.js hook A           -> run only Hook A
...
type HookResult = { hook: string; pass: boolean; detail: string };

const HOOK_SEQUENCE_DEFAULT = ['A', 'B', 'I', 'C', 'D', 'E', 'J'] as const;
...
let TV_STUB_PORT: number;
let REMOTE_STUB_PORT: number;
let HEALTH_TIMEOUT_MS: number;
let HOOK_B_TIMEOUT_MS: number;
let POLL_INTERVAL_MS: number;
let START_SERVER_WAIT_MS: number;
let START_STUBS_WAIT_MS: number;
...
const state: {
  ...
  start: number;
  exiting: boolean;
} = { processes: {}, results: [], start: Date.now(), exiting: false };

// Resolve platform-specific executables
const NPM = process.platform === 'win32' ? 'npm.cmd' : 'npm';
// const NPX = process.platform === 'win32' ? 'npx.cmd' : 'npx'; // reserved

function log(msg: string) {
  if (DEBUG) console.log(`[validate] ${msg}`);
}

function spawnProc(key: string, command: string, args: string[], opts: { cwd?: string; env?: NodeJS.ProcessEnv } = {}) {
  try {
    ...
    proc.on('exit', (code) => {
      if (!state.exiting) log(`${key} exited with code ${code}`);
    });
    return proc;
  } catch (e: any) {
...
async function httpOk(path: string, timeoutMs: number): Promise<boolean> {
  const deadline = Date.now() + timeoutMs;
  while (Date.now() < deadline) {
    const ok = await new Promise<boolean>((res) => {
      const req = http.get({ hostname: 'localhost', port: SERVER_PORT, path }, (r) => {
        r.resume();
...
async function fetchJson(port: number, path: string): Promise<any | null> {
...
async function postJson(port: number, path: string, body: any): Promise<{ status: number; json: any | null }> {
  return new Promise((resolve) => {
...
function getTvState() {
  return fetchJson(TV_STUB_PORT, '/state');
}
function getTvHealth() {
  return fetchJson(TV_STUB_PORT, '/health');
}
function getVersion(s: any): number | undefined {
  return s?.lastStateSync?.payload?.version;
}

async function waitFor(predicate: () => Promise<boolean>, timeoutMs: number, intervalMs: number) {
  const deadline = Date.now() + timeoutMs;
  while (Date.now() < deadline) {
    try {
...
async function hookA(): Promise<HookResult> {
...
async function hookB(): Promise<HookResult> {
  ...
  while (Date.now() < deadline && !(tv && remote)) {
    ...
    const tvVersion = (tvState as any)?.lastStateSync?.payload?.version;
    const remoteVersion = (remoteState as any)?.lastStateSync?.payload?.version;
    if (typeof tvVersion === 'number' && tvVersion >= 1) {
      tv = true;
      tvV = tvVersion;
    }
    if (typeof remoteVersion === 'number' && remoteVersion >= 1) {
      remote = true;
...
async function placeholder(name: string): Promise<HookResult> {
...
async function hookD(): Promise<HookResult> {
  const before = await getTvState();
  const v0 = getVersion(before) || 0;
  ...
  const ok = await waitFor(async () => {
    const s = await getTvState();
    const p = (s as any)?.lastStateSync?.payload;
    return p && p.player?.isPlaying === true && (getVersion(s) || 0) >= v0 + 1;
  }, HOOK_B_TIMEOUT_MS, POLL_INTERVAL_MS);
  if (!ok) {
    const s = await getTvState();
    const p = (s as any)?.lastStateSync?.payload;
...
async function hookE(): Promise<HookResult> {
  const before = await getTvState();
  const v0 = getVersion(before) || 0;
  ...
  if (!ok)
    return { hook: 'E', pass: false, detail: `final nav not reflected; level=${(after as any)?.lastStateSync?.payload?.navigation?.currentLevel} video=${(after as any)?.lastStateSync?.payload?.navigation?.videoId}` };
...
async function hookI(): Promise<HookResult> {
  const before = await getTvState();
  const v0 = getVersion(before) || 0;
  ...
  const ok = await waitFor(async () => {
    const s = await getTvState();
    const p = (s as any)?.lastStateSync?.payload;
    return p && p.data && (getVersion(s) || 0) >= v0 + 1;
  }, HOOK_B_TIMEOUT_MS, POLL_INTERVAL_MS);
  if (!ok) {
    const s = await getTvState();
    return { hook: 'I', pass: false, detail: `seed not reflected; v=${getVersion(s)} data=${JSON.stringify((s as any)?.lastStateSync?.payload?.data)}` };
  }
...
async function hookJ(): Promise<HookResult> {
  const before = await getTvState();
  const v0 = getVersion(before) || 0;
...
async function runHook(name: string): Promise<HookResult> {
  switch (name) {
    case 'A':
      return hookA();
    case 'B':
    ...
    case 'J':
      return hookJ();
    default:
      return { hook: name, pass: false, detail: 'Unknown hook' };
  }
}
...
async function startEnvironment() {
  ...
  try {
    ...
    SERVER_PORT = Number(WEBSOCKET_CONFIG.SERVER_DEFAULT_PORT);
    const ack = Number(WEBSOCKET_CONFIG.ACK_TIMEOUT);
  ...
  try {
    ...
    TV_STUB_PORT = Number(VALIDATION_CONFIG.STUB_PORTS.tv);
    REMOTE_STUB_PORT = Number(VALIDATION_CONFIG.STUB_PORTS.remote);
    const base = Number(VALIDATION_CONFIG.RECONNECT.BASE_MS);
    const max = Number(VALIDATION_CONFIG.RECONNECT.MAX_MS);
...
async function runSequence(list: readonly string[]) {
  for (const h of list) {
...
async function main() {
  if (ARG_MODE === 'hook' && !ARG_PARAM) {
    console.error('Specify hook letter, e.g. node dist/src/validate.js hook A');
    process.exit(2);
  }
  const sequence = ARG_MODE === 'hook' ? [ARG_PARAM] : (HOOK_SEQUENCE_DEFAULT as unknown as string[]);
