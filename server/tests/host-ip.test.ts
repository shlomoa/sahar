...
test('GET /host-ip returns JSON with ip and port', async () => {
  const body = await new Promise<string>((resolve, reject) => {
    http.get({ hostname: 'localhost', port: WEBSOCKET_CONFIG.SERVER_DEFAULT_PORT, path: '/host-ip' }, (res) => {
      let data = '';
  ...
  assert.equal(parsed.port, WEBSOCKET_CONFIG.SERVER_DEFAULT_PORT, 'port field should match server port');
});
